<?php
function twitter_latest_tweets_render( $count, $rts= true, $ats= true, $pop = 0, $screen_name='' ){
    try {
        if( ! function_exists('twitter_api_get') ){
            if (file_exists(BEAU_CORE_PLUGIN_BASE.'/libs/wp-twitter-api/twitter-api.php')) {
                require_once (BEAU_CORE_PLUGIN_BASE.'/libs/wp-twitter-api/twitter-api.php');
            }
        ...
        foreach( $tweets as $tweet ){
            ...
            $final = apply_filters('latest_tweets_render_tweet', $html, $date, $link, $tweet ,$handle);
            if( $final === $html ){
                //$final = '<p class="tweet-text message">'.$html.'</p><div class="clearfix"></div>'.
                 //        '<p class="tweet-details time-up">'.$date.'</p>';

                $final =  '
                    <div class="twitter-item-01">
                        <h3 class="twitter-item__content">
                            "'.$html.'"
                        </h3>
                        <span class="twitter-item--time">'.$date.'</span>
                    </div>   
                ';
            }
            $rendered[] = $final;
...
function twitter_follow_tweets( $count, $rts= true, $ats= true, $pop = 0, $screen_name='' ){
    try {
        if( ! function_exists('twitter_api_get') ){
            if (file_exists(BEAU_CORE_PLUGIN_BASE.'/libs/wp-twitter-api/twitter-api.php')) {
                require_once (BEAU_CORE_PLUGIN_BASE.'/libs/wp-twitter-api/twitter-api.php');
